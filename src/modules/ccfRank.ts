...
class CCFRankService {
    ...
    getEntry(name: string): CCFEntry | null {
        ...
        safeLog(`[CCF Match] Original: "${original}" -> Normalized: "${normalized}"`);

        // 策略 1: 精确匹配简称（大小写不敏感）
        // 适用场景：用户输入 "cvpr" 或 "CVPR"
        let entry = this.abbrsMap.get(normalized);
        if (entry) {
            safeLog(`[CCF Match] Found by abbr exact: ${entry.abbr} -> ${entry.rank}`);
            return entry;
        }
        ...
        if (entry) {
            safeLog(`[CCF Match] Found by fullName exact: ${entry.abbr} -> ${entry.rank}`);
            return entry;
        }
        ...
        for (const [key, value] of this.abbrsMap.entries()) {
            if (key.length >= 4 && normalized.includes(key)) {
                safeLog(`[CCF Match] Found by abbr fuzzy: "${normalized}" contains "${key}" -> ${value.abbr} ${value.rank}`);
                return value;
            }
        }
        ...
        for (const [key, value] of this.fullNamesMap.entries()) {
            if (key.length >= 20 && normalized.includes(key)) {
                safeLog(`[CCF Match] Found by fullName fuzzy: "${normalized}" contains "${key}" -> ${value.abbr} ${value.rank}`);
                return value;
            }
        }
    ...
    private getEntryFromItem(item: Zotero.Item): CCFEntry | null {
        ...
        try {
            ...
            if (itemType === 'conferencePaper') {
                ...
                safeLog(`[CCF] Conference paper proceedingsTitle: "${proceedingsTitle}"`);
                if (proceedingsTitle) {
                ...
                safeLog(`[CCF] Conference paper publicationTitle: "${publicationTitle}"`);
                if (publicationTitle) {
            ...
            if (itemType === 'journalArticle') {
                const publicationTitle = item.getField('publicationTitle') as string;
                safeLog(`[CCF] Journal article publicationTitle: "${publicationTitle}"`);
                if (publicationTitle) {
                    const entry = this.getEntry(publicationTitle);
                    if (entry) return entry;
                }
